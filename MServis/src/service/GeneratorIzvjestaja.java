...
public class GeneratorIzvjestaja {
    ...
    public void racun(ArrayList<StavkaRacuna> listaStavki, double ukupnaCijena, double pdv) throws JRException {

        List<Map<String, ?>> dataSource = new ArrayList<Map<String, ?>>();
        for (StavkaRacuna stavka : listaStavki) {
            Map<String, Object> m = new HashMap<String, Object>();
            m.put("id", stavka.getIdArtikla());
            m.put("idRacun", stavka.getIdRacuna() );
            m.put("naziv", stavka.getNaziv());
            m.put("kolicina", stavka.getKolicina());
        ...
        OutputStreamExporterOutput exporterOutput = new SimpleOutputStreamExporterOutput("Racuni" + File.separator + listaStavki.get(0).getIdRacuna() + "racun.pdf");
        exporter.setExporterOutput(exporterOutput);
        SimplePdfExporterConfiguration configuration = new SimplePdfExporterConfiguration();
        exporter.setConfiguration(configuration);
        exporter.exportReport();
    ...
    public void racunServisa(ArrayList<StavkaServisa> listaStavki, double ukupnaCijena, double pdv) throws JRException {
